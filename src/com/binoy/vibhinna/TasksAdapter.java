...
public class TasksAdapter extends SimpleCursorAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            holder.title = (TextView) convertView.findViewById(R.id.task_queue_row_title);
            holder.message = (TextView) convertView.findViewById(R.id.task_queue_row_message);
            holder.icon = (ImageView) convertView.findViewById(R.id.task_queue_row_image);
            holder.progress = (ProgressBar) convertView.findViewById(R.id.task_queue_row_progess);
            holder.percent = (TextView) convertView.findViewById(R.id.task_queue_row_percent);
        ...
        String title;
        switch (mCursor.getInt(mCursor.getColumnIndex(DatabaseHelper.TASK_TYPE))) {
            case VibhinnaService.TASK_TYPE_NEW_VFS:
                title = mContext.getString(R.string.task_create_vfs,
                        mCursor.getString(mCursor.getColumnIndex(DatabaseHelper.TASK_VS)));
                break;
            case VibhinnaService.TASK_TYPE_FORMAT_VFS:
                title = mContext.getString(R.string.task_format_vfs,
                        mCursor.getString(mCursor.getColumnIndex(DatabaseHelper.TASK_VS)));
                break;
            case VibhinnaService.TASK_TYPE_RESIZE_VFS:
                title = mContext.getString(R.string.task_resize_vfs,
                        mCursor.getString(mCursor.getColumnIndex(DatabaseHelper.TASK_VS)));
                break;
            default:
        ...
        switch (mCursor.getInt(mCursor.getColumnIndex(DatabaseHelper.TASK_STATUS))) {
