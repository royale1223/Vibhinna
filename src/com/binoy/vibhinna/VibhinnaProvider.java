...
public class VibhinnaProvider extends ContentProvider {
    ...
    public static final Uri CONTENT_URI = Uri.parse("content://" + AUTHORITY + "/" + VFS_BASE_PATH);
    public static final Uri LIST_DISPLAY_URI = Uri.parse("content://" + AUTHORITY + "/"
            + VFS_BASE_PATH + "/list");
    ...
    static {
        ...
        sURIMatcher.addURI(AUTHORITY, VFS_BASE_PATH + "/details/#", VFS_DETAILS);
    ...
    public int delete(Uri uri, String where, String[] selectionArgs) {
        ...
        switch (sURIMatcher.match(uri)) {
            case VFS:
                count = mDatabase.delete(DatabaseHelper.VFS_DATABASE_TABLE, where, selectionArgs);
                break;
            case VFS_ID:
                count = mDatabase.delete(DatabaseHelper.VFS_DATABASE_TABLE, BaseColumns._ID + " = "
    ...
    public Uri insert(Uri uri, ContentValues values) {
        long rowID = mDatabase.insert(DatabaseHelper.VFS_DATABASE_TABLE, null, values);
        if (rowID > 0) {
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        switch (sURIMatcher.match(uri)) {
            case VFS:
                count = mDatabase.update(DatabaseHelper.VFS_DATABASE_TABLE, values, selection,
                        selectionArgs);
                break;
            case VFS_ID:
                count = mDatabase.update(DatabaseHelper.VFS_DATABASE_TABLE, values, BaseColumns._ID
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (uriType) {
                ...
                if (c.moveToFirst()) {
                    do {
                        ...
                        if (root.canRead()) {
                            ...
                            fsii[5] = mContext.getString(R.string.vfs_short_info, cache, data,
                                    system);
                            fsii[6] = vdstatus;
            ...
            case VFS_DETAILS:
                ...
                Cursor databaseCursor = mDatabase.query(DatabaseHelper.VFS_DATABASE_TABLE,
                        Constants.allColumns, "_id = ?", new String[] { uri.getLastPathSegment() },
                        null, null, null);
                ...
                try {
                    ...
                    scanner.findWithinHorizon(
                            Pattern.compile("Filesystem\\smagic\\snumber:\\s*(\\S+)"), 0);
                    ...
                    vsinfo[8] = Integer.parseInt(chblockcount) * Integer.parseInt(chblocksize)
                            / 1048576 + "";
                    vsinfo[9] = Integer.parseInt(chfreeblocks) * Integer.parseInt(chblocksize)
                            / 1048576 + "";
                    vsinfo[10] = chblockcount;
                    vsinfo[11] = chfreeblocks;
                    vsinfo[12] = chblocksize;
                } catch (Exception e) {
                    Log.w("Exception", "exception in executing :" + Constants.CMD_TUNE2FS + vspath
                            + "/cache.img");
                }
                try {
                    ...
                    vsinfo[16] = Integer.parseInt(dablockcount) * Integer.parseInt(dablocksize)
                            / 1048576 + "";
                    vsinfo[17] = Integer.parseInt(dafreeblocks) * Integer.parseInt(dablocksize)
                            / 1048576 + "";
                ...
                try {
                    ...
                    vsinfo[24] = Integer.parseInt(syblockcount) * Integer.parseInt(syblocksize)
                            / 1048576 + "";
                    vsinfo[25] = Integer.parseInt(syfreeblocks) * Integer.parseInt(syblocksize)
                            / 1048576 + "";
